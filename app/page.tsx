 ...
 * to set the baseline before the more dynamic surfaces come in. */
    ...
    <main className="mx-auto max-w-5xl px-6 py-16 sm:py-24">
      <Inspect mode="static-route" note="/ — full static route">
        <header>
          ...
          <h1 className="display mt-6 text-5xl sm:text-7xl">anchor.</h1>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-[var(--color-ink-dim)]">
            An AI-native product catalog. Every item below has a human page and
            an LLM-facing endpoint optimized for citation, discovery, and agent
      ...
      <nav className="mt-16 flex flex-wrap gap-x-6 gap-y-2 border-t border-[var(--color-rule)] pt-6 text-sm">
        ...
        <Link href="/agents" className="text-[var(--color-ink-dim)] hover:text-[var(--color-accent)]">
          → For agents
        </Link>
        <Link href="/dashboard" className="text-[var(--color-ink-dim)] hover:text-[var(--color-accent)]">
          → Live AEO dashboard
        </Link>
        <Link href="/docs/rendering" className="text-[var(--color-ink-dim)] hover:text-[var(--color-accent)]">
          → Rendering docs
        </Link>
      </nav>
      ...
      <footer className="mt-24 border-t border-[var(--color-rule)] pt-8 text-sm text-[var(--color-ink-dim)]">
        <p>
          ...
          <a className="underline decoration-[var(--color-accent)]" href="https://forge.kevinmurphywebdev.com">
            forge
          </a>{" "}
          and{" "}
          <a className="underline decoration-[var(--color-accent)]" href="https://loom.kevinmurphywebdev.com">
            loom
          </a>
          .
        </p>
